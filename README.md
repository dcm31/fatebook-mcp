...
await client.getQuestions.list({ apiKey: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.getQuestions.list({ apiKey: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: getQuestions, response: raw } = await client.getQuestions
  .list({ apiKey: 'REPLACE_ME' })
  .withResponse();
...
1. Via the `FATEBOOK2_LOG` environment variable
...
const client = new Fatebook2({
  logger: logger.child({ name: 'Fatebook2' }),
...
const client = new Fatebook2({ fetch });
```
...
```ts
import Fatebook2 from 'npm:fatebook2';
